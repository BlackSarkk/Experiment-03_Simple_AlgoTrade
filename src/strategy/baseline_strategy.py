"""
Baseline Algorithmic Strategy Engine.
Implements the exact audited rule-based ETH strategy signal generation.
Shared identically across Backtest and Paper Forward Test modes.
"""
...
@dataclass
class Signal:
    candle_idx: int                       # Index of completed signal candle
    timestamp: int                        # Timestamp of completed candle
    datetime_str: str                     # Human-readable UTC datetime
    signal_type: str                      # 'LONG' or 'SHORT'
    close_price: float                    # Close price at signal candle
    ema_51: float                         # 51 EMA value
    rsi: float                            # RSI value
    atr: float                            # ATR value
    sl_price: float                       # Calculated Stop-Loss price
    tp_price: float                       # Calculated Take-Profit price
    risk_per_unit: float                  # Absolute dollar risk per unit (Entry - SL)
    consolidation_detected: bool          # Consolidation confirmation
    volume: float                         # Candle volume
    vol_sma_20: float                     # 20 Volume SMA
    swing_high: float                     # Swing High at signal
    swing_low: float                      # Swing Low at signal
    reason: str                           # Signal trigger summary


class BaselineStrategy:
    """Implements the complete audited ETH rule-based strategy."""
    ...
    def generate_signals(self, df: pd.DataFrame) -> List[Signal]:
        ...
        # Precompute RSI oversold/overbought lookback (6 candles)
            ...
            if ema_cross_up and rsi_long_valid and cons_long_valid and trend_long_ok and slope_long_ok and vol_long_ok:
                ...
                if risk_dist > 0:
                    ...
                    signals.append(Signal(
                        ...
                        volume=float(vols[i]),
                        vol_sma_20=float(vol_smas[i]),
                        swing_high=float(sw_high),
                        swing_low=float(sw_low),
                        reason="Bullish 51 EMA crossover with RSI, Consolidation & Volume confirmation"
                    ))
                    continue
            ...
            if ema_cross_down and rsi_short_valid and cons_short_valid and trend_short_ok and slope_short_ok and vol_short_ok:
                ...
                if risk_dist > 0:
                    ...
                    signals.append(Signal(
                        ...
                        swing_high=float(sw_high),
                        swing_low=float(sw_low),
                        reason="Bearish 51 EMA crossover with RSI, Consolidation & Volume confirmation"
                    ))
